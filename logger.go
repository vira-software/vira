...
// Log records key-value pairs for structured
...
//
//	log := Log{}
//	Info(log.KV("key1", "foo").KV("key2", 123))
...
//	Err(redisLog.Into(Log{"data": "foo"}))
...
//	if ctx.Path == "/" {
//		log.Reset() // reset log, don't for path "/"
//	} else {
//		log["data"] = someData
//	}
func (l Log) Reset() {
...
//
//	127.0.0.1 - - [2024-03-27T12:23:13.161Z] "GET /context.go?query=xxx HTTP/1.1" 200 21559 5.228ms
func developmentConsume(log Log, ctx *Context) {
...
func NewLogger(w io.Writer) *Logger {
	...
	logger.init = func(log Log, ctx *Context) {
		...
		if s := ctx.GetHeader(HeaderUpgrade); s != "" {
			log["upgrade"] = s
		}
		if s := ctx.GetHeader(HeaderOrigin); s != "" {
			log["origin"] = s
		}
		if s := ctx.GetHeader(HeaderReferer); s != "" {
			log["referer"] = s
		}
		if s := ctx.GetHeader(HeaderXCanary); s != "" {
			log["xCanary"] = s
		}
		log["userAgent"] = ctx.GetHeader(HeaderUserAgent)
	}

	logger.consume = func(log Log, ctx *Context) {
		end := time.Now().UTC()
		log["duration"] = end.Sub(ctx.StartAt) / 1e6 // ms

		if s := ctx.GetHeader(HeaderXRequestID); s != "" {
			log["xRequestId"] = s
		} else if s := ctx.Res.Get(HeaderXRequestID); s != "" {
			log["xRequestId"] = s
		}

		if router := GetRouterPatternFromCtx(ctx); router != "" {
...
// A Logger represents an active object that generates lines of
// output to an io.Writer. Each operation makes a single call to
...
//	logger.SetLogInit(func(log Log, ctx *Context) {
//	  log["ip"] = ctx.IP().String()
//	  log["method"] = ctx.Method
//	  log["uri"] = ctx.Req.RequestURI
//	  log["proto"] = ctx.Req.Proto
...
//	  }
//	})
//	logger.SetLogConsume(func(log Log, _ *Context) {
//		end := time.Now().UTC()
...
type Logger struct {
	...
	// something more adventorous, such as to Kafka.
	Out     io.Writer
	json    bool
	l       Level               // level
	tf, lf  string              // time format, log format
	mu      sync.Mutex          // ensures atomic writes; protects the following fields
	init    func(Log, *Context) // hook to initialize log with Context
	consume func(Log, *Context) // hook to consume log
}
...
func (l *Logger) SetLevel(level Level) *Logger {
	...
	defer l.mu.Unlock()
	if level > DebugLevel {
		panic(ViraErr.WithMsg("invalid logger level"))
	}
	l.l = level
	return l
}
...
func (l *Logger) SetLogInit(fn func(Log, *Context)) *Logger {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.init = fn
	return l
...
// It will be called on a "end hook" and should write the log to underlayer system.
...
func (l *Logger) SetLogConsume(fn func(Log, *Context)) *Logger {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.consume = fn
	return l
...
// New implements Any interface,then we can use ctx.Any to retrieve a Log instance from ctx.
// Here also some initialization work after created.
func (l *Logger) New(ctx *Context) (any, error) {
	log := Log{}
	l.init(log, ctx)
	return log, nil
}
...
func (l *Logger) FromCtx(ctx *Context) Log {
	any, _ := ctx.Any(l)
	return any.(Log)
}
...
//
//	app.Use(func(ctx *Context) error {
//		SetTo(ctx, "Data", []int{1, 2, 3})
//		return ctx.HTML(200, "OK")
//	})
func (l *Logger) SetTo(ctx *Context, key string, val any) {
	any, _ := ctx.Any(l)
	any.(Log)[key] = val
}

// Serve implements Handler interface, we can use logger as middleware.
//
//	app := New()
//	app.UseHandler(Default())
//	app.Use(func(ctx *Context) error {
//		log := FromCtx(ctx)
//		log["data"] = []int{1, 2, 3}
//		return ctx.HTML(200, "OK")
//	})
func (l *Logger) Serve(ctx *Context) error {
	...
	ctx.OnEnd(func() {
		...
		if ctx.Res.Status() == 500 {
			if body, _ := ctx.Any("vira_REQUEST_BODY"); body != nil {
				if b, ok := body.([]byte); ok {
					log["requestBody"] = string(b)
					if contentType, _ := ctx.Any("vira_REQUEST_CONTENT_TYPE"); contentType != nil {
			...
			if b := ctx.Res.Body(); b != nil {
				...
				log["responseContentType"] = ctx.Res.Get(HeaderContentType)
			}
		}
...
//		return ctx.HTML(200, "OK")
//	})
func SetTo(ctx *Context, key string, val any) {

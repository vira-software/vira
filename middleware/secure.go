...
// Default provides protection for your Vira app by setting
...
func DNSPrefetchControl(allow bool) vira.Middleware {
	return func(ctx *vira.Context) error {
		if allow {
			ctx.SetHeader(vira.HeaderXDNSPrefetchControl, "on")
		} else {
			ctx.SetHeader(vira.HeaderXDNSPrefetchControl, "off")
		}
		return nil
	}
}
...
func FrameGuard(action FrameGuardAction, domains ...string) vira.Middleware {
	if action == FrameGuardActionAllowFrom && len(domains) != 1 {
		panic(vira.ViraErr.WithMsg("'X-Frame-Options: ALLOW-FROM' only support one domain"))
	}

	return func(ctx *vira.Context) error {
		switch action {
		case FrameGuardActionDeny:
			ctx.SetHeader(vira.HeaderXFrameOptions, "DENY")
		case FrameGuardActionSameOrigin:
			ctx.SetHeader(vira.HeaderXFrameOptions, "SAMEORIGIN")
		case FrameGuardActionAllowFrom:
			ctx.SetHeader(vira.HeaderXFrameOptions, "ALLOW-FROM "+domains[0])
		}
		return nil
	}
}
...
func HidePoweredBy() vira.Middleware {
	return func(ctx *vira.Context) error {
		ctx.After(func() {
			ctx.Res.Header().Del(vira.HeaderXPoweredBy)
		})
		return nil
	}
}
...
func PublicKeyPinning(options PublicKeyPinningOptions) vira.Middleware {
	if len(options.Sha256s) == 0 {
		panic(vira.ViraErr.WithMsg("empty Public-Key-Pins sha256s"))
	}
	...
	return func(ctx *vira.Context) error {
		...
		if options.ReportOnly {
			ctx.SetHeader(vira.HeaderPublicKeyPinsReportOnly, publicKeyPins)
		} else {
			ctx.SetHeader(vira.HeaderPublicKeyPins, publicKeyPins)
		}
		return nil
	}
}
...
func StrictTransportSecurity(options StrictTransportSecurityOptions) vira.Middleware {
	return func(ctx *vira.Context) error {
		...
		ctx.SetHeader(vira.HeaderStrictTransportSecurity, val)
		return nil
	}
}
...
func IENoOpen() vira.Middleware {
	return func(ctx *vira.Context) error {
		ctx.SetHeader(vira.HeaderXDownloadOptions, "noopen")
		return nil
	}
}
...
func NoSniff() vira.Middleware {
	return func(ctx *vira.Context) error {
		ctx.SetHeader(vira.HeaderXContentTypeOptions, "nosniff")
		return nil
	}
}
...
func NoCache() vira.Middleware {
	return func(ctx *vira.Context) error {
		ctx.SetHeader(vira.HeaderCacheControl, "private, no-cache, max-age=0, s-max-age=0, must-revalidate")
		ctx.SetHeader(vira.HeaderPragma, "no-cache")
		ctx.SetHeader(vira.HeaderExpires, "0")
		return nil
	}
}
...
func SetReferrerPolicy(policy ReferrerPolicy) vira.Middleware {
	return func(ctx *vira.Context) error {
		ctx.SetHeader(vira.HeaderRefererPolicy, string(policy))
		return nil
	}
}
...
func XSSFilter() vira.Middleware {
	return func(ctx *vira.Context) error {
		ieVersion, err := getIEVersionFromUA(ctx.GetHeader(vira.HeaderUserAgent))
		if err == nil && ieVersion < 9 {
			ctx.SetHeader(vira.HeaderXXSSProtection, "0")
		} else {
			ctx.SetHeader(vira.HeaderXXSSProtection, "1; mode=block")
		}
		return nil
	}
}
...
func ContentSecurityPolicy(directives CSPDirectives) vira.Middleware {
	return func(ctx *vira.Context) error {
		csp := ""
		...
		if directives.ReportOnly {
			ctx.SetHeader(vira.HeaderContentSecurityPolicyReportOnly, csp)
		} else {
			ctx.SetHeader(vira.HeaderContentSecurityPolicy, csp)
		}
		return nil
	}
}
